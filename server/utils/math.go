...
func FloatEqual(a, b float64) bool {
	const epsilon = 1e-9 // 允许的误差范围，可根据业务调整
	return math.Abs(a-b) < epsilon
}

func CompareBalances(dbBalance float64, chainBalance int64) bool {
	const epsilon = 1e-8 // 根据业务需求调整误差范围
	return math.Abs(dbBalance-float64(chainBalance)) < epsilon
}
...
func HoursSince(unixMillis int64) float64 {
	// 当前时间
	now := time.Now()
	// 将毫秒时间戳转为 time.Time
	targetTime := time.UnixMilli(unixMillis)
	// 计算时间差
...
func DaysSince(unixMillis int64) float64 {
	now := time.Now()
	targetTime := time.UnixMilli(unixMillis) // Go 1.17+ 支持
	diff := now.Sub(targetTime)
	days := diff.Hours() / 24 // 转为天数（含小数）
	return days
}
